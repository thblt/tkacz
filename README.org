...
Tkacz is strongly typed.
 ...
 - There is an *either* type, similar to Haskell's =Either=.
 - There is a *reference to entity* type.  A reference stores the identifier of an entity and its type, probably as a pair: =(NaturalPerson . MichelFoucault)=.  References are typed.
 ...
 - Classes and properties declaration include a *user-readable*, localizable name and optional documentation.  Eg: =(defclass BirthCertificate PublicRecord #name "Birth certificate" #document "A legal birth certificate, help by a Public Records office.")=.  This should automatically generate translation templates.
 ...
 - Type are *inspectable at runtime*: the GUI system needs typing data to build UIs.  Inspection doesn't have to be dynamic, since at this point types are read-only.  Names, documentation and types of properties, as well as hierarchy of types, have to be inspectable.
